...
- **Never let a client reach the API with an empty group list.** Pocket ID's `isGroupRestricted` flag is derived by the provider from `allowed_user_groups`, and an empty list turns the restriction *off* — every account gets in. The `lifecycle.precondition` in `clients.tf` and the matching check in `lint.sh` are both load-bearing; do not weaken either.
- **`gate: envoy` clients must not enable PKCE.** Envoy Gateway v1.8 sends no `code_challenge`, so PKCE breaks the gated flow. This is derived in `locals.tf` — keep it derived, not per-app.
