...
> **SUPERSEDED 2026-05-17 (Phase 6.7)**: a runtime SOPS minta teljesen megszűnt. A `cluster-secrets.sops.yaml` és a `homepage/secret.sops.yaml` is ExternalSecret-re migrálva, a `kubernetes/flux/vars/` mappa törölve, a `sops-age` Secret a `bootstrap/resources.yaml.j2`-ből kivéve. A bootstrap-time `op inject` 2 db Connect Secret-re csökkent. Az alábbi döntésleírás már nem a tényleges futó állapot. Lásd: [06-repo-restructure.md](./06-repo-restructure.md) STATUS L20-32.
...
> **SUPERSEDED 2026-05-17 (Phase 6.7)**: a megvalósítás visszatért az egyetlen `cluster-apps` Kustomization mintára (bjw-s parity). A `cluster-vars` Kustomization eltüntetve, a `kubernetes/flux/vars/` mappa törölve, a `substituteFrom` patch eltávolítva. A live `kubernetes/flux/cluster/ks.yaml` egyetlen `cluster-apps` KS-t tartalmaz, csak HelmRelease default patch-csel. Az alábbi döntésleírás már nem a tényleges futó állapot. Lásd: [06-repo-restructure.md](./06-repo-restructure.md) STATUS L20-32.
...
- **external-secrets** + 1Password Connect ClusterSecretStore (`onepassword-connect`)
...
| 009 | Secrets | SOPS+ExternalSecrets | op-inject 2 Connect Secret (bootstrap), runtime tisztán ExternalSecret (~~`sops-age` + SOPS megszüntetve Phase 6.7~~) | Chicken-and-egg fix + bjw-s parity |
| 010 | Talos config | n/a | jinja2 + op inject | Egyetlen templating layer |
| 011 | Storage | democratic-csi | democratic-csi | Változatlan |
| 012 | NVMe szétosztás | n/a | PC801 → OS+etcd, PC711 → data PVC | etcd fsync prioritás |
...
| 022 | Flux root struktúra | egyetlen `cluster-apps` Kustomization | egyetlen `cluster-apps` Kustomization (Phase 6.7 után visszaegyszerűsítve, bjw-s parity — ~~`cluster-vars` split megszüntetve~~) | bjw-s parity, kevesebb komplexitás |
