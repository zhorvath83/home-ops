...
- [observation] [decision] All native OIDC clients use the public issuer (https://idm.<PUBLIC_DOMAIN>) for every endpoint; the OIDC backchannel is ordinary gateway traffic. The oidc-backchannel vocabulary label + CCNP drafted the same day was discarded before commit.
...
## rev5 — per-gateway ingress labels (2026-07-11, implemented)

- [observation] [decision 2026-07-11] The V5(m) gateways split ships with PER-GATEWAY ingress labels that mirror the envoy gateway names 1:1, SUPERSEDING both the rev3 `gateways-dual`/`gateways-internal` model and the rev4-naming `allow-gateways-dual`/`allow-gateways-internal` amendment. Final vocabulary: `ingress.home.arpa/allow-gateway-external` (admits envoy-external only, CCNP ingress-from-gateway-external) + `ingress.home.arpa/allow-gateway-internal` (admits envoy-internal only, CCNP ingress-from-gateway-internal). Singular `gateway` — each label names exactly one envoy gateway. A dual-routed app carries BOTH labels; the "dual" concept dissolves into label composition (Cilium ingress allows are a union, so two labels == admit both). `ingress.home.arpa/prometheus` -> `allow-prometheus` rode the same staged batch. `ingress.home.arpa/gateways` + ingress-from-gateways CCNP retired. Vocabulary is now 7 labels (egress: custom-egress, allow-world, allow-gateways; ingress: allow-gateway-external, allow-gateway-internal, allow-prometheus, none).
- [observation] [rationale] Per-gateway composition is strictly more expressive than a combined dual label and reads 1:1 against the envoy gateway names, so the intent (which gateways may reach this pod) is explicit. Behaviourally identical to the rev3 dual label for dual apps (union of allows), and it removes the need for a combined-values CCNP.
- [observation] [assignment ground-truth] Carrier→gateway mapping was verified against LIVE HTTPRoute parentRefs, not the stale 2026-07-09 rev3 list. The rev3 dual set wrongly included grafana, backrest, paperless-gpt, kopia — all four are envoy-internal ONLY (get allow-gateway-internal only). 9 dual: calibre-web-automated, echo, actual, home-gallery, homepage, mealie, paperless, pingvin-share-x, wallos. 14 internal-only: 8 *arr (bazarr, maintainerr, prowlarr, qbittorrent, radarr, seerr, sonarr, subsyncarr), grafana, kube-prometheus-stack, victoria-logs, backrest, paperless-gpt, kopia.
- [observation] [implemented] 2-commit additive->retire on main: 6980d2f3c (additive, zero behavior change) + cc684029a (retire, split enforced). Verified live: 8 CCNPs Valid, show-cnp-matrix internal apps i:gateway-external ·, negative test zero envoy-external->internal-only flows, Hubble 0 policy-denied DROPPED (2 transient rollout drops self-healed). Execution detail: [[cnp-per-app-audit]] (docs/progress) Session 22.
