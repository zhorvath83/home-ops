...
scope: Install prometheus-smartctl-exporter (:9633) and prometheus-node-exporter (:9100)
  ...
  CRs following the ScrapeConfig/openwrt pattern, extend the Prometheus CiliumNetworkPolicy
  egress, and add an ATA/SATA PrometheusRule group with promtool unit tests. This
  item owns the NAS node_exporter scrape outright.
rationale: The 16 TB NAS disk holds 12 TB of backup data, already carries 7 reallocated
  sectors at 28407 power-on hours, and its scheduled self-tests have been off for
  ~1.9 years. It is monitored today only by smartd over e-mail, which is out-of-band
  from the cluster Alertmanager/Pushover plane, has no history and no dashboard.
related_areas:
- observability
...
# Wire the OMV NAS host exporters into Prometheus
