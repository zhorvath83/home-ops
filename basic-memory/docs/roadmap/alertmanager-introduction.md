...
- [followup] **DONE (2026-07-05, commit `20e119601`)** Dead-man's-switch via healthchecks.io — Watchdog route repointed from blackhole to a new `heartbeat` receiver (`repeatInterval: 5m`) using `webhookConfigs[].urlSecret` (alertmanager-secret/watchdog_ping_url). The ping URL lives in 1Password item `alertmanager` field `ALERTMANAGER_WATCHDOG_PING_URL`, surfaced into the existing `alertmanager` ExternalSecret via a second `dataFrom` extract + a `template.data` key — no new Secret/ExternalSecret. The ping UUID never enters the repo. InfoInhibitor stays on blackhole. NOTE: requires the healthchecks.io check configured (period 5m, grace ~10m) + notification channel — user to confirm.
