...
- [followup] **DONE (2026-07-05, commit `374ec7893`)** Enabled `kubernetesResources` + `kubernetesStorage` default rule groups (resource saturation: container CPU/memory vs limits, pod resource pressure, workloads without requests; storage: PVC >75%/85% full). Both flipped to true in kube-prometheus-stack helmrelease `defaultRules.rules`. Gate satisfied — node/general rules stable since the alertmanager-introduction roadmap. Reference repos (onedr0p, bjw-s) ship these on via the chart default (neither defines a `defaultRules` section); we minify, so explicit. ~12 rules, no new exporter.
