...
- [superseded] The "[open] Auto-bump ... deferred" bullet above is SUPERSEDED. Case C auto-bump follow-up CLOSED (2026-08-06): ZERO code change - not a new customManager, but the finding that the built-in Renovate `pre-commit` manager natively handles `language: node` hook `additional_dependencies` (depType `pre-commit-node`, datasource `npm`). The auto-bump has been functional since commit `55bb8fd56` (when `language: node` + `additional_dependencies: ["renovate@44.14.1"]` landed), with no extra machinery.
- [proof] Reproduce with `renovate --platform=local --dry-run=extract` (renovate@44.14.1) from the repo root: the dep extracts as depType `pre-commit-node`, depName `renovate`, currentValue `44.14.1`, datasource `npm`, packageFile `.pre-commit-config.yaml`, no `skipReason`. Verified against the committed config WITH NO customManager - count=1 - so the customManager was redundant.
- [why-not] A customManager would have been WORSE: it loses `autoMerge.json5` `matchManagers:["pre-commit"]` (minor/patch/digest automerge) and `semanticCommits.json5` `matchManagers:["pre-commit"]` (the `chore(pre-commit): hook renovate` scope) - the built-in path keeps both.
- [audit] npm-datasource audit: the `renovate` dep hits no docker/helm/github group/label/changelog/disabledDatasources rule; only the generic `type/{major,minor,patch,digest}` labels apply (all present in `.github/labels.yaml` -> label-sync stays green). No exclusion or label addition needed.
