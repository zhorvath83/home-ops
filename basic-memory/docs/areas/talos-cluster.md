...
summary: Single-node Talos Linux control plane (`k8s-cp0` at `192.168.1.11`, cluster
...
drift_risk: The control-plane node name `k8s-cp0` and IP `192.168.1.11` are duplicated
...
## Summary
The cluster is a single Talos Linux control-plane node, `k8s-cp0` at `192.168.1.11`, cluster name `main`. Control-plane scheduling is enabled (`allowSchedulingOnControlPlanes: true`) so workloads run on the same node. Kubernetes runs without kube-proxy (Cilium replaces it) and without CoreDNS (Cilium provides DNS); Talos hostDNS is enabled with `forwardKubeDNSToHost` and `resolveMemberNames`. KubePrism listens on port 7445 for the kubelet/etcd loopback API.
...
## Components
- [component] Single control-plane node — `k8s-cp0` at `192.168.1.11`, type `controlplane`, scheduling allowed (kubernetes/talos/nodes/k8s-cp0.yaml.j2:5-6 + machineconfig.yaml.j2:98-99)
- [component] Cluster identity — name `main`, endpoint `https://192.168.1.11:6443`, cert SANs include `127.0.0.1`, `192.168.1.11`, `k8s.lan` (machineconfig.yaml.j2:110-114,149-151)
...
- [component] Kubelet config — `defaultRuntimeSeccompProfileEnabled: true`, `disableManifestsDirectory: true`, `maxPods: 150`, `serializeImagePulls: false`, image `ghcr.io/siderolabs/kubelet:<KUBERNETES_VERSION>`, nodeIP validSubnets `192.168.1.0/24` (machineconfig.yaml.j2:60-69)
...
- [component] etcd — `advertisedSubnets: [192.168.1.0/24]`, metrics URL `http://0.0.0.0:2381` (machineconfig.yaml.j2:121-128)
...
- [component] Pod/Service CIDRs — pod subnet `10.244.0.0/16`, service subnet `10.245.0.0/16`, DNS domain `cluster.local`, CNI `none` (Cilium installed out-of-band) (machineconfig.yaml.j2:160-167)
...
- [claim] "The cluster is a single control-plane node `k8s-cp0` at IP `192.168.1.11` with cluster name `main` and Kubernetes API endpoint `https://192.168.1.11:6443`" (evidence: repo, ref: machineconfig.yaml.j2:110-114,149-151 + nodes/k8s-cp0.yaml.j2:5-6, verified: 2026-05-19)
...
- [claim] "API server cert SANs are `127.0.0.1`, `192.168.1.11`, and `k8s.lan` — the `k8s.lan` name is kept as a forward-compatibility hook for a future LAN DNS record" (evidence: repo, ref: machineconfig.yaml.j2:110-114, verified: 2026-05-19)
...
- [claim] "Pod CIDR is `10.244.0.0/16`, service CIDR is `10.245.0.0/16`, DNS domain `cluster.local`, CNI is set to `none` (Cilium installed out-of-band during the bootstrap chain)" (evidence: repo, ref: machineconfig.yaml.j2:160-167, verified: 2026-05-19)
...
## Drift Risk
- [drift] The control-plane node name `k8s-cp0` and IP `192.168.1.11` are duplicated across the machineconfig (`certSANs`, `controlPlane.endpoint`), the resolver script's `FALLBACK` value, the `nodes/k8s-cp0.yaml.j2` filename, and OpenWRT DHCP reservations. The resolver script's header explicitly references rename precedents (commits `8de1fa5cc`, `19d5c9fe5`) — any future rename must keep all five locations in sync.
...
- [drift] `schematic.yaml` mentions a future Plex hardware-transcode setup (i915 + i915.enable_guc=3 + intel-ucode + mei) but the actual device mount into the Plex Pod is intentionally **not** wired today; the readiness lives at the OS level only. The cross-reference in the comment (`docs/migration/15-post-cutover.md, Phase 2`) points at content slated for archive/deletion.
