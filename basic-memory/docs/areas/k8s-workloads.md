...
drift_risk: HelmRelease minimal-spec is enforced only by code review, not by an automated
  ...
  NFS server 192.168.1.10 is a single point of failure for /backups and media mounts
...
Homepage dashboard metadata uses a stable set of groups: `Arr Stack`, `Media`, `Selfhosted`, `PFM` (personal finance), `Downloading`, `Infrastructure`. 16 of the 22 default apps are dashboard-enabled.
...
- [component] Canonical app shape — `ks.yaml` + `app/{ocirepository,helmrelease,externalsecret,kustomization}.yaml` plus optional `ciliumnetworkpolicy.yaml` and `config/` directory (.claude/skills/k8s-workloads/references/app-scaffolding.md)
...
- [component] OCIRepository pairing — every HelmRelease has a matching OCIRepository in the same directory with `metadata.name` equal to the HelmRelease name
...
- [component] Apps with file-level /backups/<app> export — `paperless` (canonical pattern: NFS mount of `192.168.1.10:/backups/paperless` at `/data/nas/export`, scheduled `document_exporter` job)
- [component] Apps with NFS mounts (8) — `backrest`, `bazarr`, `calibre-web-automated`, `home-gallery`, `paperless`, `plex`, `qbittorrent`, `radarr`, `resticprofile`, `sonarr`, `subsyncarr` (mix of read-only media mounts and read-write workspace mounts at `192.168.1.10`)
...
- [component] Homepage dashboard metadata — `gethomepage.dev/enabled`, `gethomepage.dev/name`, `gethomepage.dev/group`, `gethomepage.dev/icon` annotations on HTTPRoute; 16/22 default apps enabled
...
- [component] YAML anchor policy — anchors allowed for repeated scalar values (`&port`, `&host`, `&exportDir`, `&tz`, `&resources`, `&probes`, `&image`) using lowerCamelCase; forbidden as map keys (`controllers`, `persistence`, `serviceAccount`, `bindings`) or on scalar app names (kubernetes/CLAUDE.md "YAML anchor policy")
...
- [claim] "Homepage dashboard groups in active use: `Arr Stack`, `Media`, `Selfhosted`, `PFM`, `Downloading`, `Infrastructure`. 16 of 22 default apps carry `gethomepage.dev/enabled: true`" (evidence: repo, ref: `grep -r 'gethomepage.dev/group' kubernetes/apps/default/*/app/helmrelease.yaml`, verified: 2026-05-19)
...
- [claim] "Chart strategy preference is documented as: (1) official Helm chart, (2) bjw-s `app-template`, (3) custom manifests only when needed. The repo uses bjw-s app-template for nearly every default app" (evidence: repo, ref: kubernetes/CLAUDE.md "Default Patterns" + .claude/skills/k8s-workloads/references/app-scaffolding.md:9-12, verified: 2026-05-19)
...
- [drift] The NFS server `192.168.1.10` is a single point of failure for /backups exports and media mounts across at least 11 apps. If the NAS is offline, scheduled exports silently produce stale data and writes block — Restic/Backrest health checks only catch the backup-side failure 24h later.
