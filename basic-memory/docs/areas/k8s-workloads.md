...
drift_risk: HelmRelease minimal-spec is enforced only by code review, not by an automated
  ...
  is documented only in CLAUDE.md. The `gateway-oidc` gate needs no cross-namespace ReferenceGrant (the OIDC provider is a public URL, not an in-cluster Service); IAM wiring per app is the component + a Pocket ID client,
...
- [claim] "Four app-shaped subtrees under kubernetes/apps are outside this area's inventory and belong to other areas: `crowdsec/` (crowdsec + crowdsec-bouncer + crowdsec-web-ui), `security/pocket-id/` (the IAM provider, see docs/areas/iam), `system-upgrade/tuppr/` (see docs/areas/talos-cluster), and the platform subtrees. `crowdsec/crowdsec-web-ui` and `security/pocket-id` are what carry the new `Security` Homepage group" (evidence: repo, ref: `ls kubernetes/apps`, verified: 2026-08-03)

- [claim] "CNP label-selector convention — which pod label a CiliumNetworkPolicy matches, and what renames move it. (1) bjw-s `app-template` sets `global.nameOverride: {{ .Release.Name }}` (chart source `templates/common.yaml` at tag `app-template-5.0.1`), so the workload NAME and the pod `app.kubernetes.io/name` label both equal the HelmRelease `metadata.name` (release name) regardless of the controller key — single-controller renders NO suffix; the controller key only sets the `app.kubernetes.io/controller` label (rendered app-template 5.0.1 with release=crowdsec-blocklist-import under both controller-key scenarios; workload name identical, only the controller label differs). (2) Non-app-template charts set `app.kubernetes.io/name` to the CHART name, not the release name — e.g. `prometheus-pushgateway`'s pod name-label is `prometheus-pushgateway` whether or not `fullnameOverride` is set, so a release rename or `fullnameOverride` change does NOT move it (the two chart families behave oppositely on a rename). (3) Repo-wide the controller label is the more-selected selector: 14 distinct CNP files / 16 occurrences (14 in `endpointSelector`, 2 in `fromEndpoints` — `paperless-gpt` and `plex-trakt-sync` allow ingress from a sibling controller) match `app.kubernetes.io/controller`; the crowdsec CNPs select on `app.kubernetes.io/name` instead; the only non-CNP consumer of the controller label is `kubernetes/mod.just:307` (the `show-cnp-matrix` recipe). (4) Consequence: a HelmRelease rename moves the `name` label, a controller-key rename moves the `controller` label — the CNP that selects the affected pod MUST follow in the SAME commit, or the CNP's default-deny posture silently drops the traffic (the #4132 trap)" (evidence: repo, ref: `grep -rn "app.kubernetes.io/controller" --include="*.yaml" kubernetes/` = 16 hits / 14 files; `grep -rln "app.kubernetes.io/controller" --include="ciliumnetworkpolicy.yaml" kubernetes/` = 14 files; `grep -rn "app.kubernetes.io/name" --include="ciliumnetworkpolicy.yaml" kubernetes/apps/crowdsec/`; app-template chart source `templates/common.yaml` at tag `app-template-5.0.1`; rendered app-template 5.0.1 release=crowdsec-blocklist-import scenarios A+B, verified: 2026-08-06)
...
## Update — 2026-07-11: NFS-dependency scale-to-zero gating (zeroscaler)

Implemented via `docs/progress/nfs-dependency-zeroscaler` (done) + `docs/progress/prometheus-adapter` (done); re-verified against the cluster after deploy.

- [observation] New reusable `kubernetes/components/zeroscaler/` Kustomize component — a single `HorizontalPodAutoscaler` (autoscaling/v2) templated via `postBuild.substitute` (`${APP}`, `${CONTROLLER:=Deployment}`, `${ZEROSCALER_METRIC_NAME:=probe_success}`, `${ZEROSCALER_JOB_NAME:=nfs_probe}`): minReplicas 0, maxReplicas 1, External metric target Value "1", behavior scaleDown/scaleUp Pods policies periodSeconds 15. Verbatim match of the bjw-s `components/zeroscaler` pattern.
- [observation] 11 NFS-mount apps wired via `ks.yaml` `components: [../../../../components/zeroscaler]`: media (`calibre-web-automated`, `plex`), downloads (`bazarr`, `qbittorrent`, `radarr`, `sonarr`, `subsyncarr`), selfhosted (`backrest`, `home-gallery`, `paperless`, `resticprofile`). Each HPA verified live: minReplicas 0, maxReplicas 1, currentReplicas 1, ScalingActive=True, ValidMetricFound on `probe_success{job: nfs_probe}`.
- [observation] Enabler: `prometheus-adapter` serves `external.metrics.k8s.io` so the HPA `type: External` metric resolves (see [[prometheus-adapter]]); without it the HPAs stay in FailedGetExternalMetric.
- [observation] Companion `Probe` CRs on blackbox-exporter renamed (nas-icmp → `devices`, nfs-tcp → `nfs`) with symmetric `<name>_probe` jobNames — the zeroscaler metric selector matches `job: nfs_probe`.
