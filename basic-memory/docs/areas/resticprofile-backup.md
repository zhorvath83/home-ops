...
- [component] backrest workload — HelmRelease in `default`, chart `backrest` via OCIRepository, bjw-s app-template, image `garethgeorge/backrest` (digest-pinned), runs as UID/GID 10001 (kubernetes/apps/default/backrest/app/helmrelease.yaml)
