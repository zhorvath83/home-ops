...
drift_risk: Two LAN VIPs (192.168.1.18, .19) hardcoded in separate manifests with
  no shared variable; rate-limit-external BackendTrafficPolicy disabled due to envoy-gateway
  v1.8.0 CRD regression (re-enable on v1.9.0 GA); envoy:v1.38.0 image tag is in EnvoyProxy
  spec, not chart-managed.
---
...
- [component] Gateway/envoy-external — HTTP+HTTPS listeners, ExternalDNS target `external.horvathzoltan.me` (gateway-external.yaml)
- [component] Gateway/envoy-internal — HTTP+HTTPS listeners, LAN VIP pinned to 192.168.1.18 (gateway-internal.yaml)
...
- [component] cloudflare-tunnel — forwards `horvathzoltan.me` + `*.horvathzoltan.me` to `envoy-external.networking.svc.cluster.local` (kubernetes/apps/networking/cloudflare-tunnel/)
...
- [component] k8s-gateway — LAN split-DNS for `horvathzoltan.me`, watches HTTPRoutes filtered to GatewayClass envoy-internal, LAN VIP 192.168.1.19 (k8s-gateway/app/helmrelease.yaml)
- [component] CiliumLoadBalancerIPPool/default — LAN VIP allocation range 192.168.1.15-25 (kube-system/cilium/config/pool.yaml)
...
- [claim] "envoy-internal Gateway is pinned to LAN VIP 192.168.1.18 via lbipam.cilium.io/ips annotation" (evidence: repo, ref: gateway-internal.yaml:23-24, verified: 2026-05-19)
- [claim] "k8s-gateway Service is pinned to LAN VIP 192.168.1.19 via loadBalancerIP chart value" (evidence: repo, ref: k8s-gateway/app/helmrelease.yaml:32, verified: 2026-05-19)
- [claim] "LAN VIPs allocated from CiliumLoadBalancerIPPool/default with range 192.168.1.15-25 inclusive" (evidence: repo, ref: cilium/config/pool.yaml:7-11, verified: 2026-05-19)
...
- [claim] "Both Gateways expose HTTP (port 80, Same-namespace routes) and HTTPS (port 443, All-namespace routes, TLS Secret 'horvathzoltan-me') listeners" (evidence: repo, ref: gateway-external.yaml:27-43 + gateway-internal.yaml:25-41, verified: 2026-05-19)
...
- [claim] "Public domain managed by this stack is horvathzoltan.me; ExternalDNS target on Gateway/envoy-external is external.horvathzoltan.me" (evidence: repo, ref: gateway-external.yaml:21 + k8s-gateway/app/helmrelease.yaml:13, verified: 2026-05-19)
...
- [gap] Router-side requirements (conditional forward `horvathzoltan.me` → 192.168.1.19, DNS rebind allowance) live outside repo — operationally documented in source readme but not reproducible from manifests alone; intent-class claim
