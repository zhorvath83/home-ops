...
- [verified] **The internal gateway's client IP is also correct on real LAN traffic (2026-08-15).** envoy-internal served 1557 requests from `192.168.1.100` and 233 from `192.168.1.110`, with `downstream_remote_address` equal to `downstream_direct_remote_address` on every line — which is what `numTrustedHops: 0` must produce, and what the earlier external-only XFF check did not cover. Traffic sample across both gateways: no 5xx at all, no request tripped the new 10s header/handshake timeouts, no `ext_authz_denied`, and `xds_nack_total` still has no series (no config was ever rejected). The only non-`-` response flag was 2× `DC` on the internal gateway, both `response_code: 0` / `duration_ms: 0` / `bytes_sent: 0` `downstream_remote_disconnect` — a client closing a connection before any HTTP request completed, out of ~1870 requests. Benign.
- [observation] **`ssl_connection_error: 2` on envoy-internal is self-inflicted**, from the `openssl -tls1_2` probes used to prove the TLS 1.3 floor. No organic client failed a handshake, so no LAN device was locked out by the raised floor.
