...
## Summary
All Cloudflare-side resources for the public domain are declared as Terraform under `provision/cloudflare/`. The configuration covers one zone (the variable `CF_DOMAIN_NAME`, currently set to `horvathzoltan.me` per the live cluster networking model), one Cloudflared tunnel (`CF_TUNNEL_NAME`), Zero Trust Access apps with Google OAuth + service token, a Workers script serving MTA-STS policy with Workers KV backing, R2 bucket for downloads with custom domain, mail-stack DNS (MX/SPF/DKIM/DMARC/MTA-STS/TLSRPT/SMTP2GO), Cloudflare-managed transforms, WAF rules for the Flux webhook (GitHub CIDR allowlist), zone hardening (SSL strict, TLS 1.2 floor, TLS 1.3 + 0-RTT, HTTP/3, DNSSEC, Bot Management), and Pushover-email-gateway notification policies.
...
- [component] Cloudflare zone — `cloudflare_zone.domain` with `type = "full"`, name from `CF_DOMAIN_NAME`, account from `CF_ACCOUNT_ID` (provision/cloudflare/main.tf:51-57)
...
- [component] Tunnel DNS CNAME — `external.<domain>` → `<tunnel-id>.cfargotunnel.com` proxied (provision/cloudflare/tunnel.tf:18-26)
- [component] Zero Trust Access apps — `Private Cloud` (`*.<domain>`, service-token + unrestricted-users policies, 720h session), `Private Cloud Photos` (`fenykepek.<domain>`, restricted-users policy), `Private website` (`www.<domain>`, bypass), `Private R2 downloads` (`downloads.<domain>`, bypass), `Flux webhook` (`flux-webhook.<domain>`, GitHub-CIDR bypass), `MTA-STS policy` (bypass), `Exchange rates` (`arfolyam.<domain>`, bypass) (provision/cloudflare/access.tf:131-225)
...
- [component] MTA-STS Workers stack — `cloudflare_workers_kv_namespace.mta_sts` + `cloudflare_workers_kv.mta_sts` (key `policy`) + `cloudflare_workers_script.mta_sts_policy` (script source at `resources/mta_sts.js`, binding `POLICY_NAMESPACE`) + `cloudflare_workers_route` on `mta-sts.<domain>/*` (provision/cloudflare/workers.tf:1-31)
- [component] R2 downloads bucket — `cloudflare_r2_bucket.downloads` location `EEUR` with custom domain `downloads.<domain>`, min TLS 1.2 (provision/cloudflare/r2_bucket.tf:1-15)
- [component] Mail DNS records — MX (3 Zoho hosts with priorities 10/20/50), SPF, DKIM (zmail+zcal selectors), DMARC (p=reject, strict aspf/adkim), TLSRPT, MTA-STS TXT id record + proxied A/AAAA target on `mta-sts.<domain>` (provision/cloudflare/dns_records.tf, provision/cloudflare/terraform.tfvars)
...
- [component] WAF for Flux webhook — `cloudflare_list.github_hooks_cidr_list` populated from `https://api.github.com/meta` `hooks` ranges + `cloudflare_ruleset.flux_webhook_waf` blocking everything but those CIDRs on `flux-webhook.<domain>` (provision/cloudflare/firewall_rules.tf:1-33)
...
- [claim] "Flux webhook is double-protected: a Zero Trust Access app with the `CIDRbasedBypass` policy AND a zone-level WAF ruleset that blocks everything not in `github_hooks_cidr_list` on `flux-webhook.<domain>`" (evidence: repo, ref: provision/cloudflare/access.tf:191-201 + firewall_rules.tf:17-33, verified: 2026-05-19)
- [claim] "MTA-STS policy is rendered from `templates/mta_sts_policy.tpl` using the same `var.dns_mx_records` map driving the MX records, with policy id `md5(rendered_policy)` exported via the `_mta-sts` TXT record — the policy file itself is served by a Cloudflare Worker bound to `mta-sts.<domain>/*`" (evidence: repo, ref: provision/cloudflare/dns_records.tf:1-7,109-115 + workers.tf:13-31, verified: 2026-05-19)
...
- [claim] "Zone settings enforce strict SSL, TLS 1.2 floor, TLS 1.3 + 0-RTT, HTTP/3, IPv6, DNSSEC active, security level high, Bot Management `fight_mode` with JS challenge, and a zone-wide cache-bypass ruleset for any `*.<domain>` request" (evidence: repo, ref: provision/cloudflare/zone_settings.tf:2-203, verified: 2026-05-19)
...
- [claim] "R2 bucket `downloads` is in location `EEUR` and exposed via `downloads.<domain>` custom domain with min TLS 1.2; the matching Access app uses bypass-everyone (no auth)" (evidence: repo, ref: provision/cloudflare/r2_bucket.tf:1-15 + access.tf:178-188, verified: 2026-05-19)
