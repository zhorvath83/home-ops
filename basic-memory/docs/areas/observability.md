...
- [component] kube-prometheus-stack — operator + Prometheus + kube-state-metrics + node-exporter; chart, minified homelab tuning, Prometheus 7d/18GB retention on 20Gi local-hostpath PVC (bumped 2026-08-18), Alertmanager enabled (internal-gateway route, 1Gi PVC, AlertmanagerConfig with pushover receiver) (kubernetes/apps/observability/kube-prometheus-stack/app/helmrelease.yaml)
...
- [claim] "Prometheus retention is explicit: 7d / 18GB on a 20Gi democratic-csi-local-hostpath PVC" (evidence: repo, ref: kube-prometheus-stack/app/helmrelease.yaml:543-560, verified: 2026-08-18)
...
- [drift] Prometheus (7d/18GB on 20Gi) and victoria-logs (14d on 10Gi) retention are fixed sizes tuned for current volume; revisit if metric/log volume grows or the local-hostpath PVC fills. The 2026-08-18 bump from 4500MB/5Gi to 18GB/20Gi required a PVC recreate (allowVolumeExpansion=false on local-hostpath; StatefulSet volumeClaimTemplates do not resize existing PVCs).
...
## Update — 2026-08-18: Prometheus TSDB growth-trend fix (hubble metric drop + PVC recreate)

Resolved the `PrometheusTSDBGrowthTrend` alert (TSDB predicted to exceed 90% of retentionSize within 7d).

- [observation] **Root cause was high-cardinality orphan metrics, not storage.** Top series by cardinality: `hubble_port_distribution_total` (2793 series), `controller_runtime_reconcile_time_seconds_bucket` (779), the envoy-proxy job (10 820 samples) and the hubble job (3353). Total ~63 282 series across 54 scrape targets.
- [observation] **Usage audit**: only `hubble_port_distribution_total` was an orphan — not referenced by any Grafana dashboard or PrometheusRule (the `HubblePolicyDeny` alert uses `hubble_drop_total`, not this metric). The other high-cardinality metrics feed live dashboards/alerts and were kept.
- [change] **Dropped `hubble_port_distribution_total`** via a new `metricRelabelings` block on the hubble `serviceMonitor` in `kubernetes/apps/kube-system/cilium/app/helmrelease.yaml` (mirrors the existing cilium-agent/operator "drop unconsumed" pattern at lines 88-92 / 104-108). Verified live: the metric is absent from Prometheus after the cilium reconcile.
- [change] **Bumped retention 7d/4500MB/5Gi -> 7d/18GB/20Gi** in `kube-prometheus-stack/app/helmrelease.yaml:543-560` (90% of PVC, ~2Gi headroom).
- [finding] **The PVC could NOT be dynamically expanded**: `democratic-csi-local-hostpath` has `allowVolumeExpansion: false`, and StatefulSet `volumeClaimTemplates` do not resize existing PVCs (Kubernetes immutability). The first bump (commit 9bbd02d1e, 9GB/10Gi) left the live PVC at 5Gi while retentionSize became 9GB — a regression where size-based retention was effectively disabled (9GB > 5Gi PVC). Caught by live verify (verify-don't-trust), not by the manifest check.
- [change] **Recreated the PVC at 20Gi** (commit 73430fe77): scaled the `Prometheus` CR replicas 1->0 (operator-managed StatefulSet), deleted the 5Gi PVC, scaled back to 1 -> operator created a fresh 20Gi PVC. Data loss accepted (no VolSync ReplicationSource on the Prometheus PVC; ~7d metrics history gone, the TSDB repopulates). Verified live: PVC capacity 20Gi, pod 2/2 Running, the `--storage.tsdb.retention.size=18GB` arg present, Flux HR resumed (not left suspended).
- [note] local-hostpath is a directory bind-mount with no per-volume quota (capacity is metadata, not enforced), so the earlier 9GB/5Gi state would technically have functioned — but the recreate makes the live claim match the desired state and restores the size-retention safety net (18GB < 20Gi).
- [follow-up] The Prometheus PVC has no VolSync backup — acceptable for a 7d metrics store, but if long retention history becomes important, add a ReplicationSource.
